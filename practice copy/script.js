...
let renderBooks = (data) => {
  ...
  let filterBooks = () => {
    ...
    bookDivs.forEach((book) => {
      ...
      let bookColor = book.getAttribute("data-disc");
  
      let genreMatch = selectedGenre === "All Genres" || bookGenre === selectedGenre;
      let colorMatch = selectedColor === "All Colors" || bookColor === selectedColor;
  
      book.style.display = genreMatch && colorMatch ? "block" : "none";
    });
  };
  

  genreSelect.addEventListener("change", filterBooks);
  discSelect.addEventListener("change", filterBooks);